...
const emit = defineEmits<{ close: []; imported: [slug: string]; deleted: [slug: string] }>();
...
const form = reactive({
    ...
    visibility: (props.skill?.isPrivate ? 'private' : 'public') as 'public' | 'private',
});
    ...
    <div class="enki-modal-bg">
        ...
        <div v-if="submitted" class="enki-modal" @click.stop>
            <header class="enki-modal-head">
                <h2>{{ isEdit ? 'Skill updated' : (mode === 'github' ? 'Skill imported' : 'Skill created') }}</h2>
            ...
            <div class="enki-modal-body">
                <div class="enki-submit-done">
                    ...
                    <h3>{{ isEdit ? 'Skill updated.' : (mode === 'github' ? 'Imported from GitHub.' : 'Skill created.') }}</h3>
                    <p><strong>{{ submittedName }}</strong> {{ isEdit ? 'has been updated.' : 'has been added to the library.' }}</p>
                </div>
            </div>
        </div>
        ...
        <div v-else class="enki-modal" @click.stop>
            ...
            <div class="enki-modal-body">
                ...
                <div v-if="!isEdit" class="enki-mode-toggle">
                    <button type="button" :class="['enki-mode-btn', { 'is-active': mode === 'create' }]" @click="mode = 'create'">Create skill</button>
                    <button type="button" :class="['enki-mode-btn', { 'is-active': mode === 'github' }]" @click="mode = 'github'">
                ...
                <div v-if="!isEdit && mode === 'github'" class="enki-form">
                    <label class="enki-field">
                        ...
                        <input type="url" v-model="githubUrl" placeholder="https://github.com/owner/repo/tree/main" autocomplete="off" />
                    </label>
                    <p v-if="importError" class="enki-field-error">{{ importError }}</p>
                    <p class="enki-field-hint">Repo root or subdirectory URL — must contain <code>SKILL.md</code> and optionally <code>skill.yaml</code>.</p>
                </div>
                ...
                <div v-else class="enki-form">
                    <label class="enki-field">
                        ...
                        <input type="text" v-model="form.name" placeholder="e.g. Doc Outliner" />
                    </label>
                    <label v-if="!isEdit" class="enki-field">
                        <span>Slug <em>optional, auto-generated</em></span>
                        <input type="text" v-model="form.slug" placeholder="doc-outliner" />
                    </label>
                    <label class="enki-field">
                        <span>Category</span>
                    ...
                    <label class="enki-field">
                        ...
                        <textarea v-model="form.summary" placeholder="What this skill does, in one sentence." maxlength="500" rows="2" />
                    </label>
                    <label class="enki-field">
                        <span>Tags <em>comma separated</em></span>
                        <input type="text" v-model="form.tags" placeholder="rag, support, drafting" />
                    </label>
                    <div class="enki-field">
                        <span>Skill file <em v-if="isEdit">optional, replaces existing contents</em></span>
                        <div v-if="archiveFile" class="enki-upload-chosen">
                            ...
                            <button type="button" class="enki-upload-clear" @click="clearArchive" aria-label="Remove file">×</button>
                        </div>
                        <button v-else type="button" class="enki-upload-btn" @click="archiveInputRef?.click()">
                            <svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                <path d="M13 10v3H3v-3M8 2v8M5 5l3-3 3 3" />
                            </svg>
                            Choose archive…
                        </button>
                        <input ref="archiveInputRef" type="file" accept=".zip,.tar,.tar.gz,.tgz" style="display:none" @change="onArchivePick" />
                        <p class="enki-field-hint">Accepted: .zip, .tar, .tar.gz — must contain <code>SKILL.md</code> at root</p>
                    </div>
                    <fieldset class="enki-field" style="border:none;padding:0;margin:0;">
                        <span>Visibility</span>
                        <div class="enki-radiogroup">
                            <label
                                v-for="o in [{ id: 'public', label: 'Everyone', desc: 'All can see' }, { id: 'private', label: 'Private', desc: 'Only you' }]"
                                :key="o.id"
                                :class="['enki-radiochip', { 'is-on': form.visibility === o.id }]"
                            >
                                <input type="radio" name="vis" :value="o.id" v-model="form.visibility" />
                                <span class="enki-radiochip-label">{{ o.label }}</span>
                                <span class="enki-radiochip-desc">{{ o.desc }}</span>
                            </label>
                        </div>
                    </fieldset>
                    <p v-if="submitError" class="enki-field-error">{{ submitError }}</p>
                    <p v-if="deleteError" class="enki-field-error">{{ deleteError }}</p>

                    <!-- Delete confirmation -->
                    <div v-if="isEdit && confirmingDelete" class="enki-delete-confirm">
                        <p><strong>Delete this skill?</strong> This will permanently remove the skill and all of its files. This cannot be undone.</p>
                        <div class="enki-delete-confirm-actions">
                            <button type="button" class="enki-btn enki-btn--ghost" :disabled="deleting" @click="confirmingDelete = false">Cancel</button>
                            <button type="button" class="enki-btn enki-btn--danger" :disabled="deleting" @click="deleteSkill">{{ deleting ? 'Deleting…' : 'Delete skill' }}</button>
                        </div>
                    </div>
                </div>
            </div>
            <footer class="enki-modal-foot">
                <div class="enki-modal-actions">
                    ...
                    >Delete</button>
                    <button type="button" class="enki-btn enki-btn--ghost" @click="emit('close')">Cancel</button>
                    <button
                    ...
                    <button
                        ...
                        type="button"
                        class="enki-btn enki-btn--primary"
                        :disabled="!form.name.trim() || (!isEdit && !archiveFile) || submitting"
                        @click="submitSkill"
                    >{{ submitting ? (isEdit ? 'Saving…' : 'Submitting…') : (isEdit ? 'Save changes' : 'Submit') }}</button>
                </div>
            </footer>
        </div>
    </div>
