...
const author = computed(() => props.authors[props.skill.author] ?? { name: props.skill.author, members: 0, skills: 0 });
...
const openFile = computed(() => props.skill.files.find((f) => f.path === openFilePath.value));
        ...
        <div class="enki-detail-scroll">
            ...
            <header class="enki-detail-head">
                <div class="enki-detail-head-inner">
                    <Monogram :key="skill.slug" :name="formatTitle(skill.name)" :tint="skill.monogramTint" :tints="tints" :size="60" :icon="skill.categoryIcon" :color="skill.categoryColor" />
                    <div style="flex: 1">
                        <div class="enki-detail-head-top">
                            <div class="enki-detail-headtext">
                                <div class="enki-detail-eyebrow">
                                    <button type="button" class="enki-link" @click="emit('authorClick', skill.author)">
                                        {{ author.name }}
                                    </button>
                                    <span class="enki-item-dot">·</span>
                                    <span class="enki-mono-sm">{{ skill.slug }}</span>
                                    <template v-if="skill.isPrivate">
                                        <span class="enki-item-dot">·</span>
                                        <span class="enki-private-badge">
                                <svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                ...
                                Private
                                    ...
                                    <template v-if="skill.isExternal">
                                        <span class="enki-item-dot">·</span>
                                        <a :href="skill.githubUrl ?? undefined" target="_blank" rel="noopener noreferrer" class="enki-external-badge">
                                ...
                                <h1 class="enki-detail-name enki-detail-name--editable">
                            ...
                            <div class="enki-detail-actions">
                                ...
                                >
                                    <svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" :class="{ 'enki-spin': syncing }">
                                ...
                                <button
                                    v-if="skill.canStar"
                                    type="button"
                                    :class="['enki-btn', 'enki-btn--ghost', { 'is-on': skill.starred }]"
                                ...
                                >
                                    ...
                                    <span>{{ skill.starred ? 'Starred' : 'Star' }}</span>
                                    <span class="enki-btn-count">{{ skill.ratings }}</span>
                                </button>
                                <a
                ...
                <p class="enki-detail-summary">
                    {{ summaryDisplay }}<button v-if="summaryTruncated && !summaryExpanded" type="button" class="enki-summary-more" @click="summaryExpanded = true">…more</button>
                </p>
                <div v-if="skill.tags && skill.tags.length" class="enki-detail-tags">
                    <span v-for="t in skill.tags" :key="t" class="enki-tagchip">#{{ t }}</span>
                </div>
            </header>
            ...
            <div class="enki-install">
                <div class="enki-install-head">
                    ...
                    <a href="/help/agent-install" class="enki-link enki-mono-sm">Need help?</a>
                </div>
                <div class="enki-install-cmd">
                    <span class="enki-install-prompt">$</span>
                    <span class="enki-install-text">{{ installCmd }}</span>
                    <button type="button" class="enki-install-copy" @click="copyInstall">
            ...
            <nav class="enki-tabs">
                ...
                >
                    <span>{{ t.label }}</span>
                    <span v-if="t.count != null" class="enki-tab-count">{{ t.count }}</span>
                </button>
            </nav>
            ...
            <div class="enki-tabbody">
                ...
                <article v-if="tab === 'readme'" class="enki-md">
                    <dl v-if="Object.keys(skill.readmeFrontmatter).length" class="enki-frontmatter">
                        <template v-for="(val, key) in skill.readmeFrontmatter" :key="key">
                            <dt>{{ key }}</dt>
                            <dd>{{ val }}</dd>
                        </template>
                    </dl>
                ...
                <template v-else-if="tab === 'usage'">
                    <div class="enki-agent-hint">
                        <span class="enki-agent-hint-title">Tell your agent how to download these skills</span>
                        <span class="enki-agent-hint-body">Your AI agent can install skills directly from the command line. <a href="/help/agent-install" class="enki-link">Learn how to set up your agent →</a></span>
                    </div>
                ...
                <div v-else-if="tab === 'files'" class="enki-files">
                    <div class="enki-files-tree">
                        <div class="enki-files-treetop">
                            <span class="enki-mono-sm">{{ skill.slug.split('/').pop() }}/</span>
                            <span class="enki-files-count">{{ skill.files.length }} files</span>
                        </div>
                        <ul>
                            <li v-for="item in fileTreeItems" :key="item.path">
                                ...
                                >
                                    <svg
                                        ...
                                        :class="['enki-chevron', { 'enki-chevron--open': item.isExpanded }]"
                                    >
                                        <path d="M2 3.5l3 3 3-3" />
                                    </svg>
                                    <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                                        <path d="M1.5 5.5h13v7.5a.5.5 0 0 1-.5.5h-12a.5.5 0 0 1-.5-.5V5.5z" fill="#dfd9c5" stroke="currentColor" stroke-opacity=".4" stroke-width=".8"/>
                                        <path d="M1.5 5.5h4.8l1.2-2H14a.5.5 0 0 1 .5.5V6" fill="#dfd9c5" stroke="currentColor" stroke-opacity=".4" stroke-width=".8"/>
                                    </svg>
                                    <span class="enki-file-name">{{ item.name }}</span>
                                </button>
                                <!-- File row -->
                                <button
                                    v-else
                                    type="button"
                                    :class="['enki-file', { 'is-active': openFilePath === item.path }]"
                                    :style="{ paddingLeft: `${8 + item.depth * 16}px` }"
                                ...
                                >
                                    <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                                        <path
                                            ...
                                            :fill="fileIconFill(item.kind ?? '')"
                                            stroke="currentColor"
                                            stroke-opacity=".35"
                                            stroke-width=".8"
                                        />
                                        <path d="M9.5 1.5v3h3" stroke="currentColor" stroke-opacity=".35" stroke-width=".8" fill="none" />
                                    </svg>
                                    <span class="enki-file-name">{{ item.name }}</span>
                                    <span class="enki-file-size">{{ item.size }}</span>
                                </button>
                            </li>
                        </ul>
                    </div>
                    <div class="enki-files-view">
                        <div class="enki-files-bar">
                            <span class="enki-mono-sm">{{ openFile?.path }}</span>
                            <span class="enki-files-meta">{{ openFile?.size }} · {{ openFile?.kind }}</span>
                        </div>
                        <pre class="enki-files-pre"><code>{{ filePreview }}</code></pre>
                    </div>
                </div>

                <!-- Changelog / Activity -->
                <div v-else-if="tab === 'changelog'" class="enki-changelog">
                    <div v-if="skill.activityLog.length === 0" class="enki-empty">
                        ...
                        >
                            ...
                            <div class="enki-activity-body">
                                <div class="enki-activity-head">
                                    <span class="enki-activity-event">{{ eventLabel(entry.event) }}</span>
                                    <span v-if="entry.causer" class="enki-activity-by">by {{ entry.causer }}</span>
                                </div>
                                <time :datetime="entry.at" class="enki-activity-time" :title="new Date(entry.at).toLocaleString()">
                                    {{ entry.atHuman }}
                                </time>
                            </div>
                        </li>
