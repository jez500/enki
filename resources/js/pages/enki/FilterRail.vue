...
<template>
    <aside class="enki-rail">
        <div class="enki-rail-section">
            ...
            <ul class="enki-cats">
                <li v-for="c in categories" :key="c.id">
                    <button
                        type="button"
                        :class="['enki-cat', { 'is-active': category === c.id }]"
                        @click="emit('update:category', c.id)"
                    >
                        <span>{{ c.label }}</span>
                        <span class="enki-cat-count">{{ counts[c.id] ?? 0 }}</span>
                    </button>
                </li>
            </ul>
        </div>
        ...
        <div class="enki-rail-section">
            ...
            <label class="enki-check">
                <input
                    type="checkbox"
                    :checked="showStarred"
                    @change="emit('update:showStarred', ($event.target as HTMLInputElement).checked)"
                />
            ...
            <label class="enki-check">
                <input
                    ...
                    @change="emit('update:showMySkills', ($event.target as HTMLInputElement).checked)"
                />
                <span class="enki-check-box">
                    <svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M3.5 8.5l2.8 2.8L12.8 4.8" />
        ...
        <div class="enki-rail-foot">
            ...
            <div class="enki-rail-meta enki-rail-meta--mono">v2026.04 · stable</div>
        </div>
    </aside>
</template>
