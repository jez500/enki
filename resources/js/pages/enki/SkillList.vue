    ...
    <section class="enki-list-col">
        <div class="enki-list-head">
            ...
            <span class="enki-list-hint">
                <kbd class="enki-kbd">↑</kbd><kbd class="enki-kbd">↓</kbd> to navigate
            </span>
        </div>
        ...
        <div class="enki-list" ref="listRef">
                ...
                <ul>
                    <li v-for="s in skills" :key="s.slug">
                        <div
                            ...
                            tabindex="0"
                            :data-slug="s.slug"
                            :class="['enki-item', { 'is-active': selectedSlug === s.slug }]"
                            @click="emit('update:selectedSlug', s.slug)"
                            @keydown.enter.prevent="emit('update:selectedSlug', s.slug)"
                            @keydown.space.prevent="emit('update:selectedSlug', s.slug)"
                        >
                            <Monogram :name="formatTitle(s.name)" :tint="s.monogramTint" :tints="tints" :size="36" :icon="s.categoryIcon" :color="s.categoryColor" />
                            <div class="enki-item-body">
                                <div class="enki-item-row1">
                                    <span class="enki-item-name">
                                        {{ formatTitle(s.name) }}
                                        <svg v-if="s.isPrivate" class="enki-item-lock" width="11" height="11" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-label="Private" title="Private">
                                    ...
                                    <button
                                        v-if="s.canStar"
                                        type="button"
                                        :class="['enki-star', { 'is-on': s.starred }]"
                                    ...
                                    >
                                        <svg width="14" height="14" viewBox="0 0 16 16" aria-hidden="true">
                                            <path
                                                ...
                                                :fill="s.starred ? 'currentColor' : 'none'"
                                                stroke="currentColor"
                                                stroke-width="1.4"
                                ...
                                <div class="enki-item-summary">{{ s.summary }}</div>
                                <div class="enki-item-meta">
                                    <span class="enki-item-slug">{{ s.slug }}</span>
                                    <span class="enki-item-dot">·</span>
                                    <span>{{ s.updated }}</span>
                                    <template v-if="s.isExternal">
                                        <span class="enki-item-dot">·</span>
                                        <span class="enki-item-external-label">External</span>
                                    </template>
                                    <template v-if="s.isPrivate">
                                        <span class="enki-item-dot">·</span>
                                        <span class="enki-item-private" title="Private skill">
            ...
            <ul v-if="skills.length === 0">
                <li class="enki-empty">
                    ...
                    <div class="enki-empty-sub">Try clearing the search or category.</div>
                </li>
            </ul>
        </div>
    </section>
